...
export default function Home() {
  ...
  const draw = () => {
    ...
    const xSquares = Math.ceil(window.innerWidth / 16)
    const ySquares = Math.ceil(window.innerHeight / 16)
    ...
    for (let i = 0; i < xSquares; i++) {
      for (let j = 0; j < ySquares; j++) {
        ...
        ctx.fillRect((i * 16) + (ticks.current), (j * 16) + (ticks.current), 12, 12);
      }
    }
  }
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    canvasElm.current.width = window.innerWidth
    canvasElm.current.height = window.innerHeight
    ...
    drawLoop.current = setInterval(() => { draw() }, 500)
    return () => drawLoop.current.clearTimeout?.()

  }, [])


  return (
    <div className='bg-[#F6F5E2] relative'>
      <canvas ref={canvasElm} width={150} height={150} className="fixed w-full h-full border-8 border-black z-0"></canvas>
      <main className="flex flex-col min-h-screen relative z-10">
        ...
        <section className="flex gap-x-4 text-[#2C2C2C] w-full justify-end">
          <div className="md:mr-[calc(20%-96px)] w-[30%] my-[148px] after:content-[''] after:z-0 after:absolute after:inset-0 overflow-visible after:size-[calc(100%+24px)] after:bg-black after:bg-opacity relative">
